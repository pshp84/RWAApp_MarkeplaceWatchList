...
export const AssetCard = ({ asset, onQuickView, onBuyClick }: AssetCardProps) => {
  ...
  const isFavorite = useAssetStore((state) => state.isFavorite(asset.id));
      ...
      <div className="flex flex-col h-full">
        ...
        >
          ...
          <motion.button
            ...
            className="absolute top-3 left-3 p-2 rounded-full bg-white/85 backdrop-blur-sm shadow-md hover:bg-white transition-colors"
          >
            <Heart
              size={18}
              className={`transition-colors ${isFavorite ? 'text-red-500 fill-red-500' : 'text-neutral-500'}`}
            />
          </motion.button>
