...
export default function MarketplacePage() {
  ...
  const isFavorite = useAssetStore((state) => state.isFavorite);
  ...
  // Assets the user has favorited that actually exist in this catalog —
  // the source of truth for both the toggle badge and the Header count.
  const favoritedAssets = assets.filter(asset => isFavorite(asset.id));

  useEffect(() => {
    // All filters combine (AND): category, price and verified narrow the
    // catalog first, then Favorites and search narrow that result further —
    // same behavior as every other filter in this bar.
    let result = filterAssets(selectedCategory, priceRange, verifiedOnly);
  ...
  }, [assets, searchQuery, selectedCategory, priceRange, verifiedOnly, favoritesOnly, favoriteIds, sortOption]);
      ...
      <div className="container-custom py-8">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-start md:items-center mb-6">
          ...
          <div className="flex items-center gap-2 w-full md:w-auto">
            ...
            >
              <Heart size={18} className={favoritesOnly ? 'fill-red-500 text-red-500' : ''} />
              Favorites
              {favoritedAssets.length > 0 && (
                <span
                  className={`min-w-[1.25rem] px-1 text-xs font-semibold rounded-full tabular-nums ${
                    favoritesOnly ? 'bg-red-100 text-red-700' : 'bg-neutral-200 text-neutral-700'
                  }`}
                >
                  {favoritedAssets.length}
                </span>
              )}
            </button>
